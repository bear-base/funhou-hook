...
def _coerce_level(value: str) -> Level:
    if value not in {"info", "warning", "danger"}:
...
def _load_channel(data: dict[str, Any]) -> ChannelConfig:
    ...
    levels = tuple(_coerce_level(item) for item in data.get("levels", ["info", "warning", "danger"]))
...
def load_config(path: Path | None = None) -> FunhouConfig:
    ...
    rules = tuple(HardRule(match=item["match"], level=_coerce_level(item["level"])) for item in raw_rules)
