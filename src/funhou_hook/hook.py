...
def _load_pending_approvals() -> dict[str, dict[str, str]]:
    # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state load start
    if not APPROVAL_STATE_PATH.exists():
        # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state missing
        return {}

    raw_text = APPROVAL_STATE_PATH.read_text(encoding="utf-8")
    # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state read
    try:
    ...
    # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state load done
    return state


def _save_pending_approvals(state: dict[str, dict[str, str]]) -> None:
    APPROVAL_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
    # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state save start
    ...
    try:
        ...
        if temp_path.exists():
            temp_path.unlink()
    # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state save done
...
def _recover_broken_state(raw_text: str, exc: Exception) -> None:
    ...
    # TODO(state-audit-redesign): State/Audit Log として記録
    _save_pending_approvals({})
    get_logger(LogKind.Operational).error(
        "Approval state corruption detected",
        extra={
            "state_id": str(APPROVAL_STATE_PATH),
            "error_type": type(exc).__name__,
            "detail": (
...
def _put_pending_approval(
    ...
    # TODO(state-audit-redesign): State/Audit Log として再構築 - pending approval put start
    ...
    # TODO(state-audit-redesign): State/Audit Log として再構築 - pending approval put done
...
def _pop_pending_approval(
    ...
    if tool_use_id is not None:
        # TODO(state-audit-redesign): State/Audit Log として再構築 - pop by tool_use_id
        pending = state.pop(tool_use_id, None)
        if pending is not None:
            # TODO(state-audit-redesign): State/Audit Log として再構築 - matched tool_use_id
            _save_pending_after_pop(state)
            return pending, "tool_use_id"

    # TODO(state-audit-redesign): State/Audit Log として再構築 - pop by fallback key
    pending = state.pop(fallback_key, None)
    if pending is not None:
        # TODO(state-audit-redesign): State/Audit Log として再構築 - matched fallback key
        _save_pending_after_pop(state)
        return pending, "fallback_key"

    # TODO(state-audit-redesign): State/Audit Log として再構築 - pending approval not found
    return None, "none"
...
def _save_pending_after_pop(state: dict[str, dict[str, str]]) -> None:
    if state:
        ...
        _save_pending_approvals({})
        # TODO(state-audit-redesign): State/Audit Log として再構築 - approval state cleared
